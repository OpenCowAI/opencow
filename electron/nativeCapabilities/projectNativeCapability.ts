    ...
    return [
      this.tool({
        ...
        execute: async ({ args }) => {
          ...
          const filtered = args.includeArchived ? all : all.filter((p) => !p.archivedAt)
      ...
      this.tool({
        ...
        execute: async ({ args }) => {
          const project = await this.projectService.getById(args.id)
          if (!project) {
            return this.errorResult(new Error(`Project not found: ${args.id}`))
          }

          // Fetch issue stats
          const issues = await this.issueService.listIssues({ projectId: args.id })
